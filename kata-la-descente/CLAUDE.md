...
**En-tête** — un commentaire de changelog. Version courante : **V0.1**. Le tenir à jour à
...
**Moteur** — sous `FIN TEXTE`. `jouerChoix()` applique le coût, puis le rechargement, puis
...
**Équilibre de la pile.** Le chemin gagnant dépense 58 % avant le changement de piles, qui
remet à 100 %, puis 33 % jusqu'à la sortie. Les 42 % de marge avant le rechargement sont le
...
2. **Le chemin gagnant** — la suite d'indices `1,1,0,2,0,2,0,1,0,0,2,0,1,1,0,2,2,2,2`
   depuis l'écran-titre doit aboutir à « Retour au jour », avec 72 % de pile restante.
3. **Rendu et console** — zéro erreur console, aucun débordement horizontal à 390 px
   ni à 1100 px.
